...
Mention gating defaults live per provider (`whatsapp.groups`, `telegram.groups`, `imessage.groups`, `discord.guilds`, `slack.channels`, `rocketchat.rooms`). When these allowlists are set, include `"*"` to allow all groups/rooms.
...
- `telegram`, `discord`, `slack`, `rocketchat`, `signal`, `imessage` (provider restarts)
...
| `{{Provider}}` | Provider hint (whatsapp|telegram|discord|slack|rocketchat|imessage|webchat|…) |
